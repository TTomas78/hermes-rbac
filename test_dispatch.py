...
class TestNoMutation:
    """Anti-regresion: el dispatch NUNCA muta source.user_id.

    Bug historico: el plugin mutaba source.user_id al ID canonico para
    unificar memoria/roles, pero el core persiste la sesion con ese ID
    DESPUES del hook → la sesion quedaba contaminada con el ID de otra
    plataforma y el proximo mensaje fallaba con "sin roles" (deny).
    El mapeo a canonica es interno del plugin; el source queda intacto.
    """

    def test_source_user_id_no_se_muta_con_identidad_vinculada(
            self, setup, tmp_path, monkeypatch):
        identities = tmp_path / "identities.yaml"
        identities.write_text(yaml.safe_dump({
            "persons": {
                "alice": {
                    "canonical": "discord:111111111",
                    "identities": ["discord:111111111",
                                   "telegram:222222222"],
                },
            },
        }), encoding="utf-8")
        identity_mod = _load("identity")
        monkeypatch.setattr(plugin, "_identity",
                            identity_mod.IdentityResolver(config_path=identities))

        # El canonical (discord:111111111) debe tener roles para
        # que el allow pase; el alias telegram no figura en roles.yaml.
        ev = make_event(user_id="222222222", platform="telegram")
        plugin._pre_gateway_dispatch(event=ev)
        # El source debe seguir con el ID original de Telegram.
        assert ev.source.user_id == "222222222"

    def test_roles_se_resuelven_con_canonica_sin_mutar(
            self, setup, tmp_path, monkeypatch):
        """El alias telegram hereda los roles del canonical discord."""
        cfg = tmp_path / "roles2.yaml"
        cfg.write_text(yaml.safe_dump({
            "fail_closed": True,
            "bootstrap_admins": [],
            "roles": {"admin": {"toolsets": ["*"], "skills": ["*"]}},
            "users": {"discord:111111111": ["admin"]},
        }), encoding="utf-8")
        monkeypatch.setattr(plugin, "_resolver",
                            resolver_mod.RoleResolver(config_path=cfg))
        identities = tmp_path / "identities.yaml"
        identities.write_text(yaml.safe_dump({
            "persons": {
                "alice": {
